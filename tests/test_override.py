...
# --- the repeat cooldown ---------------------------------------------------
#
# The reported bug: a single favourite (hot dog, cevapcici...) that happens to
# be on offer several days running wins every single one of them, because each
# day was picked in isolation. The picker now remembers what it already chose
# this window (and what was actually ordered just before it) and prefers a
# fresh ranked dish over repeating, as long as one clears the floor.


def test_repeat_cooldown_prefers_the_next_best_over_repeating(user):
    import datetime

    favourite = add_meal("HOT DOG")
    second = add_meal("SOLATA")
    ranking.save_rating(user["id"], favourite, 95)
    ranking.save_rating(user["id"], second, 40)

    client = FakeClient(
        menus=[
            menu_row("2026-09-10", "s1", "MALICA 1", "HOT DOG", 0),
            menu_row("2026-09-11", "s2", "MALICA 1", "HOT DOG", 0),
            menu_row("2026-09-11", "s3", "MALICA 2", "SOLATA", 1),
        ],
        order_days=[
            {"order_date": "2026-09-10", "location_id": "loc1"},
            {"order_date": "2026-09-11", "location_id": "loc1"},
    ...
    assert decisions[0]["choice"]["slot_menu_id"] == "s1"
    assert decisions[1]["choice"]["slot_menu_id"] == "s3", "fresh dish wins over repeating"
    assert decisions[1]["repeated"] is False


def test_repeat_cooldown_still_orders_the_favourite_if_nothing_else_is_offered(user):
    """A repeat beats skipping the day -- it is a last resort, not a veto."""
    import datetime
    ...
    assert decisions[1]["choice"]["slot_menu_id"] == "s2"
    assert decisions[1]["repeated"] is True
    assert "ponovitev" in decisions[1]["detail"]


def test_repeat_cooldown_expires_after_the_configured_days(user, monkeypatch):
    import datetime

    monkeypatch.setattr(picker.settings, "repeat_cooldown_days", 1)
    favourite = add_meal("HOT DOG")
    second = add_meal("SOLATA")
    ranking.save_rating(user["id"], favourite, 95)
    ranking.save_rating(user["id"], second, 40)
    ...
    decisions = picker.plan(user, client, today=datetime.date(2026, 9, 9))
    # A one-day cooldown has already lapsed by the very next day.
    assert decisions[1]["choice"]["slot_menu_id"] == "s2"
    assert decisions[1]["repeated"] is False


def test_repeat_cooldown_can_be_turned_off(user, monkeypatch):
    import datetime

    monkeypatch.setattr(picker.settings, "repeat_cooldown_days", 0)
    favourite = add_meal("HOT DOG")
    second = add_meal("SOLATA")
    ranking.save_rating(user["id"], favourite, 95)
    ranking.save_rating(user["id"], second, 40)
    ...
    decisions = picker.plan(user, client, today=datetime.date(2026, 9, 9))
    assert decisions[1]["choice"]["slot_menu_id"] == "s2", "old behaviour restored"


def test_repeat_cooldown_counts_a_manual_override_too(user):
    """Ordering the favourite by hand should count exactly as much as an
    auto-pick -- otherwise an override would be a loophole around the cooldown.
    """
    import datetime
    ...
    overrides.set_override(user["id"], "2026-09-10", "s1", favourite)
    decisions = picker.plan(user, client, today=datetime.date(2026, 9, 9))
...
def test_the_day_board_also_avoids_repeating_the_auto_choice(user):
    """overrides.days() mirrors picker.plan, so its "auto" preview must too."""
    import datetime
    ...
    observe("2026-09-10", "s1", "MALICA 1", favourite, "HOT DOG", sort_order=0)
    observe("2026-09-11", "s2", "MALICA 1", favourite, "HOT DOG", sort_order=0)
    observe("2026-09-11", "s3", "MALICA 2", second, "SOLATA", sort_order=1)

    calendar = overrides.days(user, today=datetime.date(2026, 9, 10))
    by_date = {d["date"]: d for d in calendar}

    assert by_date["2026-09-10"]["auto"]["slot_menu_id"] == "s1"
    assert by_date["2026-09-11"]["auto"]["slot_menu_id"] == "s3"
