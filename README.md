...
actively dangerous here. These pairs all score 0.70–0.87 similar and are all
...
already contain 75 distinct dishes, and a term will hold around 150 — that is
...
2. **Click "Zberi jedilnik zdaj"** (collect the menu now). This reads roughly
   three weeks of menu and takes a few seconds. Until it has run there are no
   dishes, so there is nothing to rate — the dashboard says so.
...
ranking and the week preview. (The big on/off switch does *not* erase it — it
